...
WOW_CHANGES = {
    ...
    'Corn': {'mm_net':48742,'mm_long':0,'mm_short':0},
    'Soybeans': {'mm_net':-3871,'mm_long':0,'mm_short':0},
    'Sugar': {'mm_net':124969,'mm_long':0,'mm_short':0},
    'Gold': {'mm_net':-10422,'mm_long':0,'mm_short':0},
    'WTI Crude': {'mm_net':0,'mm_long':0,'mm_short':0},
    'Live Cattle': {'mm_net':4450,'mm_long':0,'mm_short':0},
    'Lean Hogs': {'mm_net':-15966,'mm_long':0,'mm_short':0},
    'Silver': {'mm_net':1511,'mm_long':0,'mm_short':0},
    'Cocoa': {'mm_net':-2010,'mm_long':0,'mm_short':0},
    'Coffee': {'mm_net':5745,'mm_long':0,'mm_short':0},
    'SRW Wheat': {'mm_net':10880,'mm_long':0,'mm_short':0},
    'HRW Wheat': {'mm_net':0,'mm_long':0,'mm_short':0},
    'Soy Oil': {'mm_net':-1114,'mm_long':0,'mm_short':0},
    'Soy Meal': {'mm_net':25345,'mm_long':0,'mm_short':0},
}
# ── Data freshness dates — patched by update scripts ────────────────────────
DATA_DATES = {
    'cot':    '2026-03-24',
    'drought':'2026-03-24',
    'noaa':   '2026-03-28',
...
def fig_cot_heatmap(metric='NetPct', bkdn='ALL', colorby='rank', highlight=''):
    ...
    if colorby == 'rank':
        # Desaturated pastel palette — numbers are always readable
        colorscale = [
            [0.00, '#5C1A1A'], [0.10, '#8B2E2E'], [0.20, '#C47070'],
            [0.30, '#E8B4B4'], [0.45, '#F5E6E6'], [0.50, '#1C2333'],
            [0.55, '#D4EDD4'], [0.70, '#7DC47D'], [0.85, '#2E7D32'],
            [1.00, '#1B4A1E'],
        ]
        zmin, zmax = 0, 100
    else:
...
app.layout = html.Div(
    ...
    children=[
        ...
        html.Div(
            ...
            children=[
                html.Span('DATA STATUS:', style={'fontFamily':'Courier New','fontSize':'10px',
                    'color':P['muted'],'letterSpacing':'2px'}),
                html.Span([html.Span('COT ',style={'color':P['muted'],'fontSize':'11px','fontFamily':'Courier New'}),
                    html.Span(fmt_date('cot'),style={'color':P['amber'],'fontSize':'11px',
                        'fontFamily':'Courier New','fontWeight':'700'})],
                    style={'background':P['panel'],'border':f'1px solid {P["border"]}',
                           'borderRadius':'4px','padding':'2px 10px'}),
                html.Span([html.Span('DROUGHT ',style={'color':P['muted'],'fontSize':'11px','fontFamily':'Courier New'}),
                    html.Span(fmt_date('drought'),style={'color':P['teal'],'fontSize':'11px',
                        'fontFamily':'Courier New','fontWeight':'700'})],
                    style={'background':P['panel'],'border':f'1px solid {P["border"]}',
                           'borderRadius':'4px','padding':'2px 10px'}),
                html.Span([html.Span('NOAA ',style={'color':P['muted'],'fontSize':'11px','fontFamily':'Courier New'}),
                    html.Span(fmt_date('noaa'),style={'color':P['blue'],'fontSize':'11px',
                        'fontFamily':'Courier New','fontWeight':'700'})],
                    style={'background':P['panel'],'border':f'1px solid {P["border"]}',
                           'borderRadius':'4px','padding':'2px 10px'}),
                html.Span([html.Span('PRICES ',style={'color':P['muted'],'fontSize':'11px','fontFamily':'Courier New'}),
                    html.Span(fmt_date('prices'),style={'color':P['text'],'fontSize':'11px',
                        'fontFamily':'Courier New','fontWeight':'700'})],
                    style={'background':P['panel'],'border':f'1px solid {P["border"]}',
                           'borderRadius':'4px','padding':'2px 10px'}),
            ]
...
def render(tab, analog_sel, state_sel, scen_sel, sort_sel,
    ...
    if tab == 'tab-full':
        ...
        ct_px  = _ov.get('Cotton')   or (round(_ct_raw,  2) if _ct_raw  and 40<_ct_raw<200   else 65.4)
        soy_px = _ov.get('Soybeans') or (round(_soy_raw, 0) if _soy_raw and 500<_soy_raw<3000 else 990)
        corn_px= _ov.get('Corn')     or (round(_corn_raw,0) if _corn_raw and 200<_corn_raw<1200 else 463)
        ...
        COT_RISK = [
            ...
            {'cat':'MM Gross Short',   'pctile':99,'value':114030,'signal':'RECORD SHORT', 'risk':'74K contracts short — largest in 20yr dataset. Any bullish catalyst triggers cascade cover.','col':P['red']},
        ...
        live_row = html.Div([
            ...
            price_kpi('Cotton D2+',   '46%',           '',     f"USDA DM · {fmt_date('drought')}",   P['red']),
            price_kpi('MM Net Short', '99th pctile',   '',     f"CFTC COT · {fmt_date('cot')}",  P['red']),
        ], style={'display':'flex','gap':'10px','flexWrap':'wrap','marginBottom':'16px'})
