...
def fig_seasonal(analog_sel='all', state_sel='BELT'):
    # Get state-specific data or fall back to belt
    st_data = STATE_SEAS.get(state_sel, STATE_SEAS['BELT'])
    st_label = STATE_LABELS.get(state_sel, 'Cotton Belt')
    st_avg   = st_data.get('avg', SEAS['avg'])
    st_2026  = st_data.get(2026, SEAS[2026])

    fig = make_subplots(rows=1, cols=2, horizontal_spacing=0.12,
        subplot_titles=[f'Weekly D2+ trajectory — {st_label}',
                        'Belt drought score by year (analogs in red)'])
    acols = {2011:P['red'],2022:P['amber'],2007:P['purple'],2012:'#F09595',2013:'#FAC775'}
    fig.add_trace(go.Scatter(x=MONTHS_LBL, y=st_avg, mode='lines',
    ...
    fig.add_trace(go.Scatter(x=MONTHS_LBL, y=st_2026, mode='lines',
        line=dict(color=P['blue'],width=3),
        fill='tozeroy', fillcolor='rgba(75,147,209,.12)',
        name=f'2026 — {st_label}', hovertemplate='2026: %{y:.1f}%<extra></extra>',
    ), row=1, col=1)
    aset = set(ANALOG_YRS)
...
def fetch_live_futures():
    """Fetch latest closing prices from Yahoo Finance (free, no API key).
    Cotton = CT=F (ICE), Corn = ZC=F (CME), Soybeans = ZS=F (CME)."""
    ...
    for ticker, (name, exchange) in tickers.items():
        try:
            ...
            live[name] = {'price': round(price, 2), 'exchange': exchange,
                          'ticker': ticker}
        except Exception as e:
            live[name] = {'price': None, 'exchange': exchange, 'ticker': ticker}
    return live
...
def render(tab, analog_sel, state_sel, scen_sel, sort_sel,
    ...
    if tab == 'tab-seas':
        return dcc.Graph(figure=fig_seasonal(analog_sel or 'all', state_sel or 'BELT'), config=cfg)
