...
# ── Acres scenarios (thousands) — state order matches STATES list ─────────
ACRES_NCC   = [5320, 805, 362, 278, 268, 290, 152, 451, 105, 179, 266, 112, 83, 60, 54, 25, 17]
ACRES_PROSP = [5520, 835, 470, 300, 340, 290, 170, 450, 110, 250, 360, 100, 83, 60, 54, 90, 145]
ACRES_VLM   = [5500, 865, 465, 320, 290, 320, 170, 475, 115, 215, 325, 135, 110, 65, 80, 30, 18]
VLM_EDIT_DATE = ''
try:
    import openpyxl as _oxl, pathlib as _pl
    _paths = [_pl.Path(__file__).parent / 'acres_estimates.xlsx',
              _pl.Path(__file__).parent / 'cot_data' / 'acres_estimates.xlsx']
    _found = next((p for p in _paths if p.exists()), None)
    if _found:
        _ws = _oxl.load_workbook(_found, data_only=True).active
        _SR = {'TX':21,'GA':6,'AR':12,'MS':14,'NC':7,'AL':4,'SC':8,'OK':20,
               'LA':13,'TN':16,'MO':15,'KS':19,'AZ':24,'VA':9,'FL':5,'NM':26,'CA':25}
        _ST = ['TX','GA','AR','MS','NC','AL','SC','OK','LA','TN','MO','KS','AZ','VA','FL','NM','CA']
        ACRES_NCC   = [int(_ws.cell(_SR[s],3).value or 0)//1000 for s in _ST]
        ACRES_PROSP = [int(_ws.cell(_SR[s],4).value or 0)//1000 for s in _ST]
        ACRES_VLM   = [int(_ws.cell(_SR[s],5).value or 0)//1000 for s in _ST]
        VLM_EDIT_DATE = str(_ws.cell(1,6).value or '')
except Exception:
    pass
...
def build_state_rows(tx_fail_loss=0, acres=None):
    if acres is None: acres = NCC_2026
    out = []
    for i, st in enumerate(STATES):
        d2, d3 = FCAST_D2[i], FCAST_D3[i]
        pen, hr = yield_model(d2, d3)
        adj_y = max(300, BASE_Y[i] + pen + (-0.8 * d3))
        prod  = max(0, (acres[i] * hr * adj_y) / 480)
        if st == 'TX': prod = max(0, prod - tx_fail_loss)
        out.append(dict(st=st, planted=acres[i], d2=round(d2,1),
...
HEATMAP_DATA  = {**_EMBEDDED_HEATMAP, **_heat_loaded} if _heat_loaded else _EMBEDDED_HEATMAP
...
COT_COMMODITIES = [
    'Cotton','Corn','Soybeans','SRW Wheat','Sugar','Soy Oil','Cocoa','Coffee'
]
...
WOW_CHANGES = {
    'Cocoa': {'mm_net':0,'mm_long':0,'mm_short':0},
    'Sugar': {'mm_net':0,'mm_long':0,'mm_short':0},
    'Coffee': {'mm_net':0,'mm_long':0,'mm_short':0},
    'Silver': {'mm_net':0,'mm_long':0,'mm_short':0},
    'Gold': {'mm_net':0,'mm_long':0,'mm_short':0},
    'Lean Hogs': {'mm_net':0,'mm_long':0,'mm_short':0},
    'Live Cattle': {'mm_net':0,'mm_long':0,'mm_short':0},
    'SRW Wheat': {'mm_net':0,'mm_long':0,'mm_short':0},
    'HRW Wheat': {'mm_net':0,'mm_long':0,'mm_short':0},
    'Corn': {'mm_net':0,'mm_long':0,'mm_short':0},
    'Soybeans': {'mm_net':0,'mm_long':0,'mm_short':0},
    'Soy Oil': {'mm_net':0,'mm_long':0,'mm_short':0},
    'Soy Meal': {'mm_net':0,'mm_long':0,'mm_short':0},
    'Cotton': {'mm_net':0,'mm_long':0,'mm_short':0},
}
...
def fetch_bloomberg_prices():
    """
    Fetch prices via Bloomberg Desktop API (BLPAPI).
    Requires: pip install blpapi  AND  Bloomberg Terminal running.
    Returns dict or None if Bloomberg not available.
    """
    try:
        import blpapi
        ...
        SECURITIES = {
            'CTDEC1 Comdty': ('Cotton',   '¢/lb',  'ICE Futures US'),
            'C DEC1 Comdty': ('Corn',     '¢/bu',  'CME Group'),
            'S NOV1 Comdty': ('Soybeans', '¢/bu',  'CME Group'),
        }
            ...
            for msg in event:
                if msg.hasElement('securityData'):
                    ...
                    for i in range(sd.numValues()):
                        sec_data = sd.getValue(i)
                        sec_name = sec_data.getElementAsString('security')
                        if sec_name in SECURITIES:
                            name, unit, exchange = SECURITIES[sec_name]
                            fd = sec_data.getElement('fieldData')
                            px = fd.getElementAsFloat('PX_LAST') if fd.hasElement('PX_LAST') else None
                            if px and px > 0:
                                results[name] = {
                                    'price': round(px, 2),
                                    'unit': unit,
                                    'exchange': exchange + ' · Bloomberg',
                                    'status': 'bloomberg',
                                }
...
def fetch_yahoo_prices():
    """Fetch prices via Yahoo Finance (free, ~15min delay for futures)."""
    import urllib.request as _ur, json as _json
    results = {}
    # 2026 crop year contract tickers on Yahoo Finance
    # CTZ26.NYB = Dec 2026 Cotton (ICE)
    # ZCZ26.CBT = Dec 2026 Corn (CME/CBOT)
    # ZSX26.CBT = Nov 2026 Soybeans (CME/CBOT)
    TICKERS = [
        # (primary ticker, fallback ticker, name, exchange label, unit)
        ('CTZ26.NYB', 'CT=F',  'Cotton',   'ICE · Dec 2026', '¢/lb'),
        ('ZCZ26.CBT', 'ZC=F',  'Corn',     'CME · Dec 2026', '¢/bu'),
        ('ZSX26.CBT', 'ZS=F',  'Soybeans', 'CME · Nov 2026', '¢/bu'),
    ]
    def _fetch_one(ticker, name, exchange, unit):
        ...
        meta   = result['meta']
        # chartPreviousClose = most recent settled close (most accurate for futures)
    ...
    for primary, fallback, name, exchange, unit in TICKERS:
        price = None
        used_ticker = primary
        try:
            price = _fetch_one(primary, name, exchange, unit)
        except Exception:
            # Primary contract may have expired or not exist — try fallback
            try:
                price = _fetch_one(fallback, name, exchange, unit)
                used_ticker = fallback
                exchange = exchange.replace('Dec Contract','Front Month').replace('Nov Contract','Front Month')
            except Exception as _e:
                results[name] = {'price': None, 'unit': unit, 'exchange': exchange,
                                 'ticker': primary, 'status': f'error: {_e}'}
                continue
        results[name] = {
            'price':    price,
            'unit':     unit,
...
def fetch_live_futures():
    ...
    1. Bloomberg Desktop API (exact, real-time) — if Terminal is running
    2. Yahoo Finance (free, ~15min delay)
    3. Manual overrides from UI (stored in dcc.Store)
    """
    # Try Bloomberg first
    bbg = fetch_bloomberg_prices()
    if bbg and len(bbg) >= 2:
        return bbg
    # Fall back to Yahoo
...
def fig_futures():
    ...
    # Yahoo Finance returns ICE cotton in ¢/lb, CME corn/soybeans in ¢/bu
    # No unit conversion needed — values are already in correct units
    live_note = []
    if ct_live and 40 < ct_live < 200:      # sanity check: cotton 40-200¢/lb
        dec_ct_live[-1]  = round(ct_live, 1)
        live_note.append(f'Cotton: {ct_live:.2f}¢/lb (ICE live)')
    if corn_live and 200 < corn_live < 1200: # sanity check: corn 200-1200¢/bu
        dec_corn_live[-1] = round(corn_live, 0)
        live_note.append(f'Corn: {corn_live:.0f}¢/bu (CME live)')
    if soy_live and 500 < soy_live < 3000:  # sanity check: soybeans 500-3000¢/bu
        nov_soy_live[-1]  = round(soy_live, 0)
        live_note.append(f'Soybeans: {soy_live:.0f}¢/bu (CME live)')
...
def fig_cot_heatmap(metric='NetPct', bkdn='ALL', colorby='rank', highlight=''):
    ...
    for cat in COT_CATS:
        ...
        for comm in COT_COMMODITIES:
            ...
            # Text in cell — value + rank indicator below
            if metric in ('Net','Long','Short'):
                txt = f"{v:+,.0f}"
            else:
                txt = f"{v:+.1f}%"
            if colorby == 'rank':
                bar_filled = round(r / 10)
                bar = '█' * bar_filled + '░' * (10 - bar_filled)
                rank_col = ('#E24B4A' if r <= 10 else '#FF7043' if r <= 25
                            else '#4CAF50' if r >= 90 else '#66BB6A' if r >= 75
                            else '#888780')
                cell_txt = f"{txt}<br><span style='font-size:9px;color:{rank_col};letter-spacing:0'>{bar} {r}th</span>"
            else:
                cell_txt = txt
            row_t.append(cell_txt)
    ...
    if colorby == 'rank':
        colorscale = [[0.0, '#1a2030'], [1.0, '#1a2030']]
        zmin, zmax = 0, 100
    else:
        colorscale = 'RdYlGn'
    ...
    fig.update_layout(
        ...
        annotations=[
            dict(x=COT_COMMODITIES[ci], y=COT_CAT_LABELS[COT_CATS[ri]],
                 ...
                 font=dict(size=11, color='white', family='Courier New, monospace'))
...
# Store for manual price overrides
price_store = dcc.Store(id='price-store', data={})
acres_store = dcc.Store(id='acres-store', data='ncc')
...
app.layout = html.Div(
    ...
    children=[
        ...
        html.Div([
            ...
            html.Div(f'2026 season  ·  USDA Drought Monitor  ·  NCC Planting Intentions  ·  CBOT Futures  ·  CFTC COT (2006–2026)  ·  COT as of {fmt_date("cot")}  ·  Drought as of {fmt_date("drought")}  ·  Prices as of {fmt_date("prices")}  ·  Updated {fmt_date("noaa")}' if DATA_DATES else '2026 season  ·  USDA Drought Monitor  ·  NCC Planting Intentions  ·  CBOT Futures  ·  CFTC COT (2006–2026)',
        ...
        html.Div([
            html.Div([
                html.Div('ACRES ESTIMATE', style={'fontSize':'10px','color':P['sub'],
                    'fontWeight':'700','letterSpacing':'1px','marginBottom':'7px'}),
                html.Div([
                    html.Button('NCC 2026', id='btn-acres-ncc', n_clicks=0,
                        style={'fontFamily':'Courier New','fontSize':'11px','fontWeight':'700',
                               'padding':'4px 10px','borderRadius':'4px','cursor':'pointer',
                               'border':'1px solid #58a6ff','background':'#1f3a5c',
                               'color':'#58a6ff','marginRight':'5px'}),
                    html.Button('Prosp. Plantings', id='btn-acres-prosp', n_clicks=0,
                        style={'fontFamily':'Courier New','fontSize':'11px','fontWeight':'700',
                               'padding':'4px 10px','borderRadius':'4px','cursor':'pointer',
                               'border':f'1px solid {P["border"]}','background':P['panel'],
                               'color':P['sub'],'marginRight':'5px'}),
                    html.Button('VLM Estimate', id='btn-acres-vlm', n_clicks=0,
                        style={'fontFamily':'Courier New','fontSize':'11px','fontWeight':'700',
                               'padding':'4px 10px','borderRadius':'4px','cursor':'pointer',
                               'border':f'1px solid {P["border"]}','background':P['panel'],
                               'color':P['sub']}),
                ], style={'marginBottom':'6px'}),
                html.Div(id='acres-detail-label',
                    children=f'{sum(ACRES_NCC)/1000:.2f}M acres · NCC 2026 planting intentions',
                    style={'fontSize':'10px','color':P['sub']}),
            ], style={'background':P['panel'],'border':f'0.5px solid {P["border"]}',
                      'borderRadius':'8px','padding':'12px 16px','flex':'1','minWidth':'200px'}),
            html.Div(id='kpi-baseline',
                children=kpi('Baseline production',f'{NAT_BASE/1000:.1f}M bales','drought-adjusted',P['amber']),
                style={'flex':'1','minWidth':'150px'}),
            html.Div(id='kpi-failure',
                children=kpi('Failure scenario',f'{NAT_FAIL/1000:.1f}M bales','<1" S.TX/CB by plant date',P['red']),
                style={'flex':'1','minWidth':'150px'}),
            kpi(f'Belt D2+ ({fmt_date("drought")})' if DATA_DATES else 'Belt D2+ (Mar 2026)', '46%','avg area severe+ drought',P['red']),
        ...
        # ── Bloomberg price input panel (always visible) ─────────────────────
        html.Div(style={
            'background':'#1A1200',
            'border':f'2px solid {P["amber"]}',
            'borderRadius':'6px','padding':'10px 16px',
            'marginBottom':'12px','display':'flex',
            'gap':'20px','flexWrap':'wrap','alignItems':'flex-end'
        }, children=[
            html.Div([
                html.Div('BLOOMBERG PRICES — Enter after market close each day',
                         style={'fontSize':'10px','color':P['amber'],'fontWeight':'700',
                                'letterSpacing':'1px','marginBottom':'8px'}),
                html.Div(style={'display':'flex','gap':'16px','flexWrap':'wrap','alignItems':'flex-end'}, children=[
                    html.Div([
                        html.Div('Dec Cotton ¢/lb',
                                 style={'fontSize':'10px','color':P['sub'],'marginBottom':'3px',
                                        'fontWeight':'600'}),
                        html.Div('CTDEC1 Comdty',
                                 style={'fontSize':'9px','color':P['muted'],'marginBottom':'4px',
                                        'fontFamily':'Courier New'}),
                        dcc.Input(id='ov-cotton', type='number', placeholder='e.g. 73.58',
                                  debounce=True, step=0.01,
                                  style={'width':'120px','background':P['bg'],'color':'#FFD700',
                                         'border':f'2px solid {P["amber"]}','borderRadius':'4px',
                                         'padding':'7px 10px','fontFamily':'Courier New',
                                         'fontSize':'16px','fontWeight':'700',
                                         'letterSpacing':'0.5px'}),
                    ]),
                    html.Div([
                        html.Div('Dec Corn ¢/bu',
                                 style={'fontSize':'10px','color':P['sub'],'marginBottom':'3px',
                                        'fontWeight':'600'}),
                        html.Div('C DEC1 Comdty',
                                 style={'fontSize':'9px','color':P['muted'],'marginBottom':'4px',
                                        'fontFamily':'Courier New'}),
                        dcc.Input(id='ov-corn', type='number', placeholder='e.g. 469',
                                  debounce=True, step=0.25,
                                  style={'width':'120px','background':P['bg'],'color':'#FFD700',
                                         'border':f'2px solid {P["amber"]}','borderRadius':'4px',
                                         'padding':'7px 10px','fontFamily':'Courier New',
                                         'fontSize':'16px','fontWeight':'700',
                    ...
                    html.Div([
                        html.Div('Nov Soybeans ¢/bu',
                                 style={'fontSize':'10px','color':P['sub'],'marginBottom':'3px',
                                        'fontWeight':'600'}),
                        html.Div('S NOV1 Comdty',
                                 style={'fontSize':'9px','color':P['muted'],'marginBottom':'4px',
                                        'fontFamily':'Courier New'}),
                        dcc.Input(id='ov-soy', type='number', placeholder='e.g. 1175',
                                  debounce=True, step=0.25,
                                  style={'width':'120px','background':P['bg'],'color':'#FFD700',
                                         'border':f'2px solid {P["amber"]}','borderRadius':'4px',
                                         'padding':'7px 10px','fontFamily':'Courier New',
                    ...
                    html.Div(id='override-status',
                             style={'fontSize':'12px','color':P['teal'],'fontWeight':'600',
                                    'alignSelf':'center','fontFamily':'Courier New'}),
                ]),
            ], style={'flex':'1'}),
            html.Div([
                html.Div('Bloomberg Terminal tickers:',
                         style={'fontSize':'9px','color':P['muted'],'marginBottom':'4px'}),
                html.Div('CTDEC1 Comdty',
                         style={'fontSize':'11px','color':P['sub'],'fontFamily':'Courier New'}),
                html.Div('C DEC1 Comdty',
                         style={'fontSize':'11px','color':P['sub'],'fontFamily':'Courier New'}),
                html.Div('S NOV1 Comdty',
                         style={'fontSize':'11px','color':P['sub'],'fontFamily':'Courier New'}),
                html.Div('pip install blpapi → auto-pulls when Terminal open',
                         style={'fontSize':'9px','color':P['muted'],'marginTop':'4px',
                                'fontStyle':'italic'}),
            ], style={'borderLeft':f'1px solid {P["border"]}','paddingLeft':'16px',
                      'minWidth':'180px'}),
        ]),
...
@app.callback(
    Output('acres-store',        'data'),
    Output('btn-acres-ncc',      'style'),
    Output('btn-acres-prosp',    'style'),
    Output('btn-acres-vlm',      'style'),
    Output('kpi-baseline',       'children'),
    Output('kpi-failure',        'children'),
    Output('acres-detail-label', 'children'),
    Input('btn-acres-ncc',   'n_clicks'),
    Input('btn-acres-prosp', 'n_clicks'),
    Input('btn-acres-vlm',   'n_clicks'),
)
def toggle_acres(n_ncc, n_prosp, n_vlm):
    from dash import ctx
    triggered = ctx.triggered_id or 'btn-acres-ncc'
    base = {'fontFamily':'Courier New','fontSize':'11px','fontWeight':'700',
            'padding':'4px 10px','borderRadius':'4px','cursor':'pointer','marginRight':'5px'}
    last = {**base, 'marginRight':'0'}
    s_ncc_on   = {**base, 'border':'1px solid #58a6ff','background':'#1f3a5c','color':'#58a6ff'}
    s_prosp_on = {**base, 'border':f'1px solid {P["teal"]}','background':'#1a2f1a','color':P['teal']}
    s_vlm_on   = {**last, 'border':f'1px solid {P["amber"]}','background':'#2a1f0a','color':P['amber']}
    s_off      = {**base, 'border':f'1px solid {P["border"]}','background':P['panel'],'color':P['sub']}
    s_off_last = {**last, 'border':f'1px solid {P["border"]}','background':P['panel'],'color':P['sub']}
    if triggered == 'btn-acres-prosp':
        acres = ACRES_PROSP
        styles = s_off, s_prosp_on, s_off_last
        detail = f'{sum(acres)/1000:.2f}M acres · USDA Prospective Plantings · Mar 31, 2026'
    elif triggered == 'btn-acres-vlm':
        acres = ACRES_VLM
        styles = s_off, s_off, s_vlm_on
        edit_str = f' · Last edited: {VLM_EDIT_DATE}' if VLM_EDIT_DATE else ''
        detail = f'{sum(acres)/1000:.2f}M acres · VLM internal estimate{edit_str}'
    else:
        acres = ACRES_NCC
        styles = s_ncc_on, s_off, s_off_last
        detail = f'{sum(acres)/1000:.2f}M acres · NCC 2026 planting intentions'
    base_rows = build_state_rows(0,   acres)
    fail_rows = build_state_rows(525, acres)
    nat_base  = sum(r['prod'] for r in base_rows)
    nat_fail  = sum(r['prod'] for r in fail_rows)
    kpi_base = kpi('Baseline production', f'{nat_base/1000:.1f}M bales','drought-adjusted',P['amber'])
    kpi_fail = kpi('Failure scenario', f'{nat_fail/1000:.1f}M bales','<1" S.TX/CB by plant date',P['red'])
    return triggered, *styles, kpi_base, kpi_fail, detail


@app.callback(
    Output('price-store', 'data'),
    Output('override-status', 'children'),
    Input('ov-cotton', 'value'),
    Input('ov-corn',   'value'),
    Input('ov-soy',    'value'),
)
def store_overrides(ct, corn, soy):
    data = {}
    parts = []
    if ct   and 40   < ct   < 200:  data['Cotton']   = ct;   parts.append(f'CT={ct:.2f}¢')
    if corn and 200  < corn < 1200: data['Corn']     = corn; parts.append(f'Corn={corn:.0f}¢')
    if soy  and 500  < soy  < 3000: data['Soybeans'] = soy;  parts.append(f'Soy={soy:.0f}¢')
    status = ('✓ Override active: ' + '  ·  '.join(parts)) if parts else ''
    return data, status
...
def render(tab, analog_sel, state_sel, scen_sel, sort_sel,
           cot_metric, cot_bkdn, cot_colorby, cot_hi, cot_comm, price_overrides):
    ...
    if tab == 'tab-full':
        ...
        # Price source priority: manual override → Bloomberg → Yahoo → fallback
        _ov      = price_overrides or {}
        _ct_raw  = live_futures.get('Cotton',   {}).get('price')
        _soy_raw = live_futures.get('Soybeans', {}).get('price')
        _corn_raw= live_futures.get('Corn',     {}).get('price')
        ct_px  = _ov.get('Cotton')   or (round(_ct_raw,  2) if _ct_raw  and 40<_ct_raw<200   else 74.03)
        soy_px = _ov.get('Soybeans') or (round(_soy_raw, 0) if _soy_raw and 500<_soy_raw<3000 else 1144)
        corn_px= _ov.get('Corn')     or (round(_corn_raw,0) if _corn_raw and 200<_corn_raw<1200 else 490)
        ...
        COT_RISK = [
            {'cat':'Managed Money Net','pctile':9,'value':-33546,'signal':'EXTREME SHORT','risk':'Contrarian bull — max short-covering fuel. 72% hit rate at 1Y when MM ≤5th pctile.','col':P['red']},
            {'cat':'MM Gross Short',   'pctile':93,'value':74951,'signal':'ELEVATED SHORT','risk':'74,951 contracts short — near 20yr highs. Any bullish catalyst triggers cascade cover.','col':P['red']},
            {'cat':'MM Gross Long',    'pctile':56,'value':41405,'signal':'NEUTRAL','risk':'Longs not crowded. Room for significant new long accumulation on price rally.','col':P['muted']},
            {'cat':'Producer Hedge',   'pctile':59,'value':-73487,'signal':'NEUTRAL HEDGE','risk':'Producer hedging near average — typical positioning for this time of year.','col':P['teal']},
        ]
        ...
        # Live price row
        _ct_src  = 'ICE Futures US · Live' if (_ct_raw and 40<_ct_raw<200) else 'ICE · Last known'
        _corn_src= 'CME Group · Live'     if (_corn_raw and 200<_corn_raw<1200) else 'CME · Last known'
        _soy_src = 'CME Group · Live'     if (_soy_raw and 500<_soy_raw<3000) else 'CME · Last known'
        live_row = html.Div([
            price_kpi('Dec Cotton',   f'{ct_px:.2f}',  '¢/lb', _ct_src,  P['amber']),
            price_kpi('Dec Corn',     f'{corn_px:.0f}','¢/bu', _corn_src, P['amber']),
            price_kpi('Nov Soybeans', f'{soy_px:.0f}', '¢/bu', _soy_src,  P['amber']),
            price_kpi('Cotton D2+',   '46%',           '',     'USDA DM · Mar 24 2026',   P['red']),
            price_kpi('MM Net Short', '99th pctile',   '',     'CFTC COT · Mar 24 2026',  P['red']),
        ], style={'display':'flex','gap':'10px','flexWrap':'wrap','marginBottom':'16px'})
        ...
        cot_risk_tbl = html.Div([
            sh('COT Positioning Risk Analysis — Mar 24, 2026'),
            ...
            html.Div([
                ...
                html.Div([
                    ...
                    html.Span('Cotton losing acres to corn at current price ratios. '
                              f'Dec cotton at {ct_px:.0f}¢ vs Dec corn at {corn_px:.0f}¢/bu favors corn on marginal Texas acres. '
    ...
    if tab == 'tab-cot-heat':
        ...
        # ── Week-over-week change summary bar ──────────────────────────────
        wow_cards = html.Div(style={'display':'flex','gap':'6px','flexWrap':'wrap','marginTop':'10px','marginBottom':'4px'}, children=[
            html.Div([
                html.Div(comm, style={'fontSize':'10px','color':P['sub'],'fontWeight':'700',
                                      'marginBottom':'3px','fontFamily':'Courier New'}),
                html.Div(
                    f"{'▲' if WOW_CHANGES.get(comm,{}).get('mm_net',0)>0 else '▼'} {WOW_CHANGES.get(comm,{}).get('mm_net',0):+,.0f}",
                    style={'fontSize':'11px','fontWeight':'700','fontFamily':'Courier New',
                           'color': P['teal'] if WOW_CHANGES.get(comm,{}).get('mm_net',0)>0 else P['red']}
                ),
                html.Div('MM Net WoW', style={'fontSize':'9px','color':P['muted']}),
            ], style={'background':P['panel'],'border':f'0.5px solid {P["border"]}',
                      'borderRadius':'5px','padding':'6px 10px','minWidth':'90px','textAlign':'center'})
            for comm in COT_COMMODITIES
        ])
        return html.Div([
    ...
    if tab == 'tab-cot-proj':
        ...
        CMETA = {
            'Cotton':      {'unit':'¢/lb',  'px':74.5,  'ag':True,  'sector':'Softs',    'contract':'ICE Dec'},
            'Corn':        {'unit':'¢/bu',  'px':485,   'ag':True,  'sector':'Grains',   'contract':'CBOT Dec'},
            'Soybeans':    {'unit':'¢/bu',  'px':1160,   'ag':True,  'sector':'Grains',   'contract':'CBOT Nov'},
        ...
        BCOM = {
            'Cotton':      {'gs':99.5, 'net_s':99.1, 'gl_asc':40.6, 'short_oi':96.0, 'net_oi':97.3, 'long_oi_asc':10.3, 'mm_net':-33546, 'mm_long':41405,  'mm_short':74951, 'prod_net':0, 'swap_net':44513,   'other_net':0},
            'Corn':        {'gs':62.1, 'net_s':57.5, 'gl_asc':59.2, 'short_oi':57.8, 'net_oi':59.1, 'long_oi_asc':45.1, 'mm_net':279630,  'mm_long':363377, 'mm_short':83747, 'prod_net':0,'swap_net':299296,  'other_net':0},
            'Soybeans':    {'gs':54.6, 'net_s':4.8,  'gl_asc':98.4, 'short_oi':41.0, 'net_oi':21.7, 'long_oi_asc':86.4, 'mm_net':191350, 'mm_long':227533, 'mm_short':36183,  'prod_net':0,'swap_net':112292,  'other_net':0},
            'SRW Wheat':   {'gs':52.0, 'net_s':40.0, 'gl_asc':88.4, 'short_oi':52.6, 'net_oi':39.7, 'long_oi_asc':87.8, 'mm_net':-994, 'mm_long':110732,  'mm_short':111726, 'prod_net':0, 'swap_net':67820,   'other_net':0},
            'HRW Wheat':   {'gs':65.9, 'net_s':49.6, 'gl_asc':82.1, 'short_oi':52.3, 'net_oi':50.4, 'long_oi_asc':60.1, 'mm_net':9518,   'mm_long':71212,  'mm_short':61694,  'prod_net':0, 'swap_net':72038,   'other_net':0},
            'Sugar':       {'gs':100.0,'net_s':100.0,'gl_asc':41.0, 'short_oi':99.9, 'net_oi':99.8, 'long_oi_asc':13.5, 'mm_net':-73170,'mm_long':149710, 'mm_short':222880, 'prod_net':0,  'swap_net':146367,  'other_net':0},
            ...
            'Gold':        {'gs':35.9, 'net_s':59.1, 'gl_asc':28.5, 'short_oi':38.3, 'net_oi':62.6, 'long_oi_asc':18.1, 'mm_net':91621, 'mm_long':119562, 'mm_short':27941,  'prod_net':0, 'swap_net':-181612, 'other_net':0},
            'Silver':      {'gs':11.3, 'net_s':75.3, 'gl_asc':0.5,  'short_oi':11.7, 'net_oi':74.2, 'long_oi_asc':0.7,  'mm_net':11158,   'mm_long':15246,  'mm_short':4088,   'prod_net':0, 'swap_net':-25046,  'other_net':0},
            'Live Cattle': {'gs':23.2, 'net_s':13.9, 'gl_asc':88.1, 'short_oi':19.0, 'net_oi':18.2, 'long_oi_asc':80.5, 'mm_net':107593, 'mm_long':122119, 'mm_short':14526,  'prod_net':0,'swap_net':55661,   'other_net':0},
            'Lean Hogs':   {'gs':27.3, 'net_s':1.5,  'gl_asc':98.7, 'short_oi':7.4,  'net_oi':3.2,  'long_oi_asc':95.4, 'mm_net':93770, 'mm_long':114103, 'mm_short':20333,  'prod_net':0,'swap_net':69248,   'other_net':0},
            'Soy Oil':     {'gs':73.5, 'net_s':11.5, 'gl_asc':100.0,'short_oi':46.3, 'net_oi':37.1, 'long_oi_asc':64.4, 'mm_net':118496,  'mm_long':148285, 'mm_short':29789,  'prod_net':0,'swap_net':69772,   'other_net':0},
            'Soy Meal':    {'gs':65.6, 'net_s':9.7,  'gl_asc':95.5, 'short_oi':45.3, 'net_oi':37.9, 'long_oi_asc':67.1, 'mm_net':106257,  'mm_long':136139, 'mm_short':29882,  'prod_net':0,'swap_net':101116,   'other_net':0},
            'Cocoa':       {'gs':69.2, 'net_s':82.0, 'gl_asc':12.4, 'short_oi':66.6, 'net_oi':81.1, 'long_oi_asc':3.1,  'mm_net':-9114,  'mm_long':29618,  'mm_short':38732,  'prod_net':0, 'swap_net':37945,   'other_net':0},
            'Coffee':      {'gs':58.2, 'net_s':51.2, 'gl_asc':46.6, 'short_oi':58.3, 'net_oi':50.5, 'long_oi_asc':61.3, 'mm_net':29997,  'mm_long':42836,  'mm_short':12839,  'prod_net':0, 'swap_net':4120,    'other_net':0},
        }
        bcom        = BCOM.get(comm, {})
        gs          = bcom.get('gs',    50.0)
        ...
        # ── Outlier / position change detection ──────────────────────────────
        wow_comm = WOW_CHANGES.get(comm, {})
        wow_net  = wow_comm.get('mm_net', 0)
        wow_long = wow_comm.get('mm_long', 0)
        wow_short= wow_comm.get('mm_short', 0)

        # Flag significant moves: >10K contracts or >3% OI shift
        oi_now = bcom.get('mm_net', 0)
        outlier_alerts = []
        if abs(wow_net) > 10000:
            direction = 'COVERING SHORTS' if wow_net > 0 else 'ADDING SHORTS'
            outlier_alerts.append({
                'title': f'MM Net Position Shift — {direction}',
                'detail': f'MM net changed {wow_net:+,.0f} contracts WoW. '
                          f'{"Short covering at this scale often signals trend reversal." if wow_net>0 else "Aggressive new shorting — bearish momentum building."}',
                'col': P["teal"] if wow_net > 0 else P["red"],
                'icon': '▲' if wow_net > 0 else '▼',
            })
        if abs(wow_long) > 8000:
            outlier_alerts.append({
                'title': f'MM Gross Long {"Build" if wow_long>0 else "Liquidation"}',
                'detail': f'Long positions changed {wow_long:+,.0f} WoW. '
                          f'{"New long money entering — watch for follow-through." if wow_long>0 else "Long liquidation — potential trend reversal risk."}',
                'col': P["teal"] if wow_long > 0 else P["amber"],
                'icon': '→',
            })
        if abs(wow_short) > 8000:
            outlier_alerts.append({
                'title': f'MM Gross Short {"Build" if wow_short>0 else "Cover"}',
                'detail': f'Short positions changed {wow_short:+,.0f} WoW. '
                          f'{"New short sellers — bearish signal, increases short-covering fuel." if wow_short>0 else "Short covering — bullish signal, reduces downside pressure."}',
                'col': P["red"] if wow_short > 0 else P["teal"],
                'icon': '◆',
            })

        outlier_section = html.Div() if not outlier_alerts else html.Div([
            html.Div('⚡ Position Change Outliers — This Week vs Last Week',
                     style={'fontSize':'11px','fontWeight':'700','color':P['amber'],
                            'textTransform':'uppercase','letterSpacing':'0.4px','marginBottom':'8px'}),
            html.Div(style={'display':'flex','gap':'10px','flexWrap':'wrap'}, children=[
                html.Div([
                    html.Div(style={'display':'flex','gap':'8px','alignItems':'center','marginBottom':'5px'}, children=[
                        html.Span(a['icon'], style={'fontSize':'16px','color':a['col']}),
                        html.Span(a['title'], style={'fontSize':'12px','fontWeight':'700','color':a['col']}),
                    ]),
                    html.Div(a['detail'], style={'fontSize':'12px','color':P['sub'],'lineHeight':'1.5'}),
                    html.Div(f"WoW: net {wow_net:+,.0f} | long {wow_long:+,.0f} | short {wow_short:+,.0f}",
                             style={'fontSize':'10px','color':P['muted'],'marginTop':'5px',
                                    'fontFamily':'Courier New'}),
                ], style={'background':P['panel'],'border':f'1px solid {a["col"]}',
                          'borderRadius':'8px','padding':'10px 14px','flex':'1','minWidth':'280px'})
                for a in outlier_alerts
            ]),
        ], style={'background':P['panel'],'border':f'0.5px solid {P["amber"]}',
                  'borderRadius':'8px','padding':'12px 14px','marginBottom':'14px'})
