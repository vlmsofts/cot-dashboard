...
# ── Data freshness dates — patched automatically by update scripts ───────────
DATA_DATES = {
    'cot':    '2026-03-24',   # updated every Friday by update_cot.py
    'drought':'2026-03-24',   # updated every Thursday by update_drought.py
    'noaa':   '2026-03-28',   # update manually when you refresh NOAA section
    'prices': '2026-03-28',   # live via Bloomberg panel / Yahoo Finance
}

def fmt_date(key):
    """Return nicely formatted 'As of Mar 24, 2026' string for any DATA_DATES key."""
    try:
        from datetime import datetime as _dt
        raw = DATA_DATES.get(key, '')
        return _dt.strptime(raw, '%Y-%m-%d').strftime('%b %d, %Y')
    except:
        return DATA_DATES.get(key, 'Unknown')
...
def render(tab, analog_sel, state_sel, scen_sel, sort_sel,
    ...
    if tab == 'tab-full':
        ...
        cot_risk_tbl = html.Div([
            sh('COT Positioning Risk Analysis — ' + fmt_date('cot') + ''),
