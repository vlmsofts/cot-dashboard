...
def fetch_live_futures():
    """
    Fetch latest futures closing prices.
    CT=F  = ICE Cotton No.2   → returns ¢/lb  (e.g. 65.40)
    ZC=F  = CME Corn          → returns ¢/bu  (e.g. 463.25)
    ZS=F  = CME Soybeans      → returns ¢/bu  (e.g. 990.50)
    Yahoo Finance returns these already in the correct contract units.
    """
    import urllib.request as _ur, json as _json
    results = {}
    TICKERS = [
        ('CT=F', 'Cotton',   'ICE Futures US', '¢/lb'),
        ('ZC=F', 'Corn',     'CME Group',      '¢/bu'),
        ('ZS=F', 'Soybeans', 'CME Group',      '¢/bu'),
    ]
    for ticker, name, exchange, unit in TICKERS:
        try:
            url = (f'https://query1.finance.yahoo.com/v8/finance/chart/{ticker}'
                   f'?interval=1d&range=2d&includePrePost=false')
            req = _ur.Request(url, headers={'User-Agent':'Mozilla/5.0 (Windows NT 10.0)'})
            with _ur.urlopen(req, timeout=6) as r:
                data = _json.loads(r.read())
            result = data['chart']['result'][0]
            meta   = result['meta']
            # Prefer regularMarketPrice, fall back to most recent close
            ...
            results[name] = {'price': None, 'unit': unit, 'exchange': exchange,
                             'ticker': ticker, 'status': f'error: {_e}'}
    return results
...
def render(tab, analog_sel, state_sel, scen_sel, sort_sel,
    ...
    if tab == 'tab-full':
        ...
        # Yahoo returns: Cotton=¢/lb, Corn=¢/bu, Soybeans=¢/bu (already correct units)
        ...
        ct_px  = round(_ct_raw,  2) if _ct_raw  and 40<_ct_raw<200   else 65.4
        soy_px = round(_soy_raw, 0) if _soy_raw and 500<_soy_raw<3000 else 990
        corn_px= round(_corn_raw,0) if _corn_raw and 200<_corn_raw<1200 else 463
